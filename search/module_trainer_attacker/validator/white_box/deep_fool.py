...
import utils
from module.attacker import deep_fool_attack
from module.validator import Validator


class DeepFoolValidator(Validator):
    ...
    def valid(self, report_freq=50):
        ...
        for step, (image, target) in enumerate(self.valid_queue):
            ...
            # add noise to batch
            image_pert = deep_fool_attack(model=self.model, image=image,
            ...
            with torch.no_grad():
                ...
                if step % report_freq == 0:
                    logging.info('deep_fool valid %03d loss=%.2f top1-acc=%.2f top5-acc=%.2f'
