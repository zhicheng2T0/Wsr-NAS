...
class AutoAttack():
    def __init__(self, model, norm='Linf', eps=.3, seed=None, verbose=True,
        ...
        if not self.is_tf_model:
            ...
            raise RuntimeError('do not support tf')
    ...
    def run_standard_evaluation(self, x_orig, y_orig, bs=250):
        if self.verbose:
            self.logger.log('using {} version including {}'.format(self.version, ', '.join(self.attacks_to_run)))

        with torch.no_grad():
            # calculate accuracy
            n_batches = int(np.ceil(x_orig.shape[0] / bs))
            robust_flags = torch.zeros(x_orig.shape[0], dtype=torch.bool, device=x_orig.device)
            self.logger.log('run standard test')
            for batch_idx in range(n_batches):
                start_idx = batch_idx * bs
                end_idx = min((batch_idx + 1) * bs, x_orig.shape[0])

                x = x_orig[start_idx:end_idx, :].to(self.device)
                y = y_orig[start_idx:end_idx].to(self.device)
    ...
    def clean_accuracy(self, x_orig, y_orig, bs=250):
        ...
        if self.verbose:
            self.logger.log('clean accuracy: {:.2%}'.format(acc / x_orig.shape[0]))
    ...
    def run_standard_evaluation_individual(self, x_orig, y_orig, bs=250):
        ...
        for c in l_attacks:
            self.logger.log('run %s' % c)
            startt = time.time()
            self.attacks_to_run = [c]
            self.run_standard_evaluation(x_orig, y_orig, bs=bs)
            self.logger.log('time attack: %.2f s' % (time.time() - startt,))
            # if verbose_indiv:
    ...
    def set_version(self, version='standard'):
        if self.verbose:
            self.logger.log('setting parameters for {} version'.format(version))
        ...
        if version == 'standard':
            ...
            if not self.norm in ['Linf', 'L2']:
                self.logger.log('"{}" version is used with {} norm: please check'.format(version, self.norm))
