...
def _deep_fool_attack(image, net, num_classes=10, overshoot=0.02, max_iter=50):
...
def deep_fool_attack(model, image, data_mean, data_std, num_classes=10, overshoot=0.02, max_iter=50):
    pert_image = []
    image = image.sub(data_mean).div(data_std)
    for img in image:
        pert_img = _deep_fool_attack(net=model, image=img, num_classes=num_classes,
