...
Experience = namedtuple('Experience', ['weights', 'loss', 'ae_loss0', 'ae_loss1', 'ae_loss2', 'ae_loss3', 'ae_loss4', 'ae_loss5', 'ae_loss6', 'ae_loss7'])
...
class Memory(object):
    ...
    def get_batch(self, batch_size=None):
        ...
        for idx in indices:
            if self.is_gae:
                ...
                ae_loss6.append(self.memory[idx].ae_loss6)
                ae_loss7.append(self.memory[idx].ae_loss7)
            else:
                #weights.append(self.memory[idx].weights)
                ...
                ae_loss6.append(self.memory[idx].ae_loss6.cuda())
                ae_loss7.append(self.memory[idx].ae_loss7.cuda())
            #loss.append(self.memory[idx].loss)
            if len(loss) >= batch_size:
                if self.is_gae:
                    batch.append(((torch.stack(weights_normal), torch.stack(weights_reduce)), torch.stack(loss)))
                else:
                    batch.append((torch.stack(weights),
                                    torch.stack(loss),
                                    torch.stack(ae_loss0),
                                    torch.stack(ae_loss1),
                                    torch.stack(ae_loss2), 
                                    torch.stack(ae_loss3), 
                                    torch.stack(ae_loss4), 
                                    torch.stack(ae_loss5),
                                    torch.stack(ae_loss6),
                                    torch.stack(ae_loss7)))
                weights_normal = []
                weights_reduce = []
                weights = []
                loss = []
    ...
    def append(self, weights, loss, ae_loss0, ae_loss1, ae_loss2, ae_loss3, ae_loss4, ae_loss5,ae_loss6,ae_loss7):
        self.memory.append(Experience(weights=weights, loss=loss, ae_loss0=ae_loss0, ae_loss1=ae_loss1, ae_loss2=ae_loss2, ae_loss3=ae_loss3, ae_loss4=ae_loss4, ae_loss5=ae_loss5, ae_loss6=ae_loss6, ae_loss7=ae_loss7))
